...
class led(QWidget, Ui_led_ui):
    def __init__(self):
        ...
        self.action_open.setIcon(QIcon('Get_Chart.png'))
        ...
        self.action_close.setIcon(QIcon('Exit.png'))
        self.menu_file.addAction(self.action_close)  # 將 Close 選項放入 File 選項裡
        self.action_close.setShortcut('Ctrl+C')
        self.menu_file.addSeparator()
        # 建立一個 Reini_serial 選項 ( QAction )
        self.action_Reini_serial = QAction('Reini_serial')
        self.action_Reini_serial.triggered.connect(self.Reini_serial)
        # 將 Reini_serial 選項放入 File 選項裡
        self.menu_file.addAction(self.action_Reini_serial)
        self.menu_file.addSeparator()
        ...
        background_color = palette.color(
            palette.ColorGroup.Normal, palette.ColorRole.Window)
...
# 變數中，可以在這裡對資料進行進一步的處理。最後，我們使用 close() 方法關閉資料庫連線和游標。
        ...
        while True:
            ...
            if self.ser.in_waiting > 0:
                ...
                if data[0] == 'a':
                    ...
                    self.SW1.setStyleSheet(
                        "color:black;background-color: orange")
                    print('a')

                if data[0] == 'q':
                    self.SW1.setStyleSheet(
                        "color:white;background-color: #455364")
    ...
    def write_data(self, temp, humi):
        ...
        cur = conn.cursor()
        cur.execute(
            "CREATE TABLE IF NOT EXISTS record (id INT AUTO_INCREMENT PRIMARY KEY, datetime DATETIME, temperature INT, humidity INT)")
        now = datetime.now()
        datetime_string = now.strftime("%Y-%m-%d %H:%M:%S")
        temperature = temp
        humidity = humi
        cur.execute("INSERT INTO record (datetime, temperature, humidity) VALUES (%s, %s, %s)",
                    (datetime_string, temperature, humidity))
        conn.commit()
        cur.close()
        conn.close()
